...
def main():
    ...
    predicate_dict = generate_predicate_dict(TRAIN_FILE_PATH)
    predicate_dict_val = generate_predicate_dict(VALID_FILE_PATH)
    predicate_dict_test = generate_predicate_dict(TEST_FILE_PATH)

    # Generate head and tail dictionaries
    head_dict, tail_dict = generate_head_tail_dicts(predicate_dict)

    # Load rules from JSON file
    with open(OUTPUT_NEGATIVE_RULES_PATH, "r") as json_file:
    ...
    save_triples(processed_triples, OUTPUT_TP_FILE_PATH, OUTPUT_SEL_TP_FILE_PATH, ORIGINAL_TRAIN_PATH, OUTPUT_TRAIN_FILE_PATH)
