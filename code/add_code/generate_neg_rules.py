...
def main():
    ...
    relation_matrix = load_relation_matrix(RELATION_MATRIX_PATH)

    # Load the relation mappings
    relation_to_number, number_to_relation = load_relation_mappings(RELATION_TO_NUMBER_PATH)
    ...
    save_json(top_relations_dict, OUTPUT_TOP1_RELATIONS_DICT_PATH)

    # Load data
    top_relations_dict = load_json(TOP1_RELATIONS_DICT_PATH)
    json_data = load_json(JSON_DATA_PATH)
    indices = load_txt(INDICES_PATH)

    # Generate new rules
    new_rules = generate_new_rules(json_data, indices, top_relations_dict)

    # Load existing rules
    existing_data = load_json(EXISTING_RULES_PATH)

    # Filter out existing rules
    filtered_rules = filter_existing_rules(new_rules, existing_data)
    # Save the final rules
    save_json(filtered_rules, OUTPUT_NEGATIVE_RULES_PATH)
